...
async function scrapeAll(page) {
  ...
  const slotMap    = parseSlotMap(attText);
...
// Reads slot→course mapping from the attendance page
// Line format: "Regular  Course Name  Theory  Faculty (ID)  SLOT  ROOM  conducted  absent  pct"
function parseSlotMap(text) {
  ...
  for (const block of blocks) {
    ...
    // Skip practicals — they use LAB slots not mapped in unified timetable
    if (type === 'Practical') continue;

    const nameM = rest.match(/(?:Regular|Elective|Mandatory)\s+(.+?)\s+(?:Theory|Practical)/);
    const name  = nameM ? nameM[1].trim() : '';

    // Slot appears right after the faculty ID in parentheses
    // e.g. "Sarada, DVL (100826)  B  B103  28  6  78.57"
    const slotM = rest.match(/\(\d+\)\s+([A-Z][A-Z0-9-]*)\s+/);
    if (!slotM) {
      console.log('parseSlotMap: no slot for', code, rest.substring(0, 100));
      continue;
    }

    const slotRaw = slotM[1].trim();

    // A slot can be composite like "A-B" meaning two slots map to same course
    const slots = slotRaw.split('-').map(s => s.trim()).filter(Boolean);

    // Room is the token right after the slot
    const roomM = rest.match(/\(\d+\)\s+[A-Z][A-Z0-9-]*\s+(\S+)\s+\d+\s+\d+/);
    const room  = roomM ? roomM[1] : '';

    for (const slot of slots) {
      slotMap[slot] = { code, name, type, room };
      console.log('parseSlotMap:', code, slot, '->', name);
    }
  }
...
// ✅ KEY FIX: parse the unified timetable from raw concatenated text
// Input looks like:
// "...FROM08:00 - 08:50...Day 1P1P2/XP3/XP4P5AAFFGL11L12Day 2BB / XG / X..."
function parseUnifiedTimetable(text) {
  // Extract times — find all HH:MM - HH:MM patterns before "Day 1"
  ...
  // Extract day rows — split on "Day N" boundaries
  // e.g. "Day 1P1P2/XP3/XP4P5AAFFGL11L12"
  const dayRows = {};

  // Match each "Day N" followed by its slots until the next "Day N" or end
  ...
  while ((match = dayPattern.exec(text)) !== null) {
    ...
    // The slots are concatenated — split on known boundaries:
    // Each slot is: one or more uppercase letters/digits, optionally followed by /X
    // e.g. P1, P2/X, A, B/X, L11, G
    ...
    while ((sm = slotPattern.exec(rawBody)) !== null) {
      const s = sm[1].trim();
      // Filter out noise words from the page
      if (['FROM','TO','LAB','SLOTS','THEORY','HOUR','DAY','ORDER'].includes(s)) continue;
      // Must be a valid slot: single letter A-G, or P+digits, or L+digits
      if (/^[A-G]$/.test(s) || /^P\d+$/.test(s) || /^L\d+$/.test(s)) {
        slots.push(s);
      }
    }
