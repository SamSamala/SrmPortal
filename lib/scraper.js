...
async function loginWithBrowser(email, password) {
  ...
  try {
    ...
    await browser.close();
    try { await redis.set(`data:${email}`, JSON.stringify(data), 'EX', 600); } catch(e) {}
...
async function navigateToPage(page, pageName) {
  ...
  await page.waitForFunction((pageName) => {
    ...
    if (pageName.includes('Academic_Planner')) return text.length > 500;
...
function parseSlotMap(attText, myTTText) {
  ...
  // ── STEP 2: parse P## slot assignments from My_Time_Table page ──
  if (myTTText) {
    console.log('Parsing My_Time_Table for lab slots...');
    const ttLines = myTTText.split('\n').map(l => l.trim()).filter(Boolean);

    for (const line of ttLines) {
      // Look for lines with a course code and a slot like P1-P5 or P16-P20
      // Format varies — log every line that has a course code for debugging
      const codeM = line.match(/(\d{2}[A-Z]{2,5}\d{3}[A-Z]{0,3})/);
      if (!codeM) continue;
      const code = codeM[1];

      // Find the practical entry we stored
      const pracKey = '__prac__' + code;
      if (!slotMap[pracKey]) continue;

      // Extract slot pattern — P followed by digits, possibly range like P1-P5
      ...
      for (let n = startSlot; n <= endSlot; n++) {
        const slotKey = 'P' + n;
        slotMap[slotKey] = { ...slotMap[pracKey] };
        console.log('Lab slot:', slotKey, '->', code, slotMap[pracKey].name);
      }

      // Also extract room from this line if available
      const roomM = line.match(/\b([A-Z][A-Za-z0-9\s,]+(?:Lab|Room|Block)[A-Za-z0-9\s,]*)/i);
      if (roomM) {
        for (let n = startSlot; n <= endSlot; n++) {
          if (slotMap['P' + n]) slotMap['P' + n].room = roomM[1].trim();
        }
      }
    }
...
function parsePlanner(text) {
  ...
  let curMonth = -1, curYear = new Date().getFullYear();

  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];

    // Detect month header: "January 2026" or "JANUARY"
    const mh = line.match(/^(january|february|march|april|may|june|july|august|september|october|november|december)\s*(\d{4})?$/i);
    if (mh) {
      curMonth = MONTHS.indexOf(mh[1].toLowerCase());
      if (mh[2]) curYear = parseInt(mh[2]);
      continue;
    }

    // Detect inline: "15 Thursday Day 3" or "15 Thursday Pongal"
    if (curMonth !== -1) {
      const pd = line.match(/^(\d{1,2})\s+(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)\s+(.+)$/i);
      if (pd) {
        const day = parseInt(pd[1]);
        const rest = pd[3].trim();
        const key = `${curYear}-${String(curMonth+1).padStart(2,'0')}-${String(day).padStart(2,'0')}`;
        _applyPlannerEntry(result, key, rest);
        continue;
      }

      // Detect single day number with info on next lines
      const justDay = line.match(/^(\d{1,2})$/);
      if (justDay) {
        const day = parseInt(justDay[1]);
        if (day >= 1 && day <= 31) {
          const ahead = lines.slice(i+1, i+4).join(' ');
          const key = `${curYear}-${String(curMonth+1).padStart(2,'0')}-${String(day).padStart(2,'0')}`;
          _applyPlannerEntry(result, key, ahead);
        }
      }
    }
  }
...
function _applyPlannerEntry(result, key, text) {
  const dom = text.match(/Day\s*(\d)/i);
  if (dom) {
    result[key] = { order: parseInt(dom[1]), holiday: null };
  } else if (text.length > 2) {
    const name = text
      .replace(/\b(Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)\b/gi, '')
      .replace(/\b(Holiday)\b/gi, '')
      .replace(/\d+/g, '').replace(/\s+/g, ' ').trim() || 'Holiday';
    result[key] = { order: null, holiday: name };
  }
}
