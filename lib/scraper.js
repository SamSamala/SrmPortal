...
async function scrapeAll(cookies) {
  ...
  async function fetchPlannerSem(sem) {
    for (const name of _plannerCandidates(sem)) {
      // Try /report/ first (list view — parseable), then /page/ (card view — JS-rendered, may be empty)
      const html = await fetchReport(cookieStr, name) || await fetchPage(cookieStr, name);
      if (html) { console.log('Planner found:', name); return html; }
    }
