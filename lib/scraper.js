...
function storageFile(email) {
  return path.join(STORAGE_DIR, email.replace(/[^a-z0-9]/gi,'_') + '.json');
}
...
async function saveSession(context, email) {
  ...
  await context.storageState({ path: storageFile(email) });
  ...
  try {
    ...
    if (currentUrl.includes('signin') || currentUrl.includes('accounts')) {
      ...
      try {
        ...
        const pwdVisible = await errFrame.$('#password, input[type="password"]').catch(function(){return null;});
        if (pwdVisible) {
          const errEl = await errFrame.$('.disp-err, [class*="error" i]').catch(function(){return null;});
          const errText = errEl ? await errEl.textContent().catch(function(){return '';}) : '';
          const file = storageFile(email);
...
async function navigateToPage(page, pageName) {
  ...
  //  Timetable page → wait for table rows
  if (pageName === 'My_Time_Table_2023_24') {
    const rows = document.querySelectorAll('table tr');
    return rows.length > 5;
  }

  //  Attendance page → text check is enough
  const text = document.body.innerText || '';
  return text.includes('Registration Number') && text.length > 300;

}, pageName, { timeout: 15000 });

const text = await page.evaluate(() => document.body.innerText);

console.log('Page "' + pageName + '" loaded fast len=' + text.length);

return text;
  console.log('Warning: "' + pageName + '" did not fully load');
...
if (useSession && fs.existsSync(file)) {
  console.log('Trying restored session for:', email);
  ...
  try {
    ...
    console.log('Session expired, doing fresh login...');
    await browser.close();
...
function extractDayRows(lines) {
  ...
  for (let i=0;i<lines.length;i++) {
    ...
    if (!dm) continue;
    const tokens=dm[2].match(/(?:[A-G]|P\d+|L\d+)(?:\s*\/\s*X)?/gi)||[];
    const slots=tokens.map(function(s){return s.replace(/\s*\/\s*X/i,'').trim().toUpperCase();}).filter(function(s){return s.length>0;});
    if (slots.length>0) dayRows[dm[1].replace(/\s+/,' ')]=slots;
  }
  console.log('Day rows:', JSON.stringify(dayRows));
  return dayRows;
}
