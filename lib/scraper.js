...
async function navigateToPage(page, pageName, attempt = 1) {
  ...
  const timeout = pageName === 'My_Attendance' ? 60000
    : pageName.includes('Academic_Planner')    ? 45000
    : 25000;
  ...
  try {
    await page.waitForFunction((pageName) => {
      ...
      if (pageName === 'My_Attendance') {
        ...
        // Wait for marks section to fully load; fall back if page is very long (no marks case)
        return text.includes('Internal Marks') || text.length > 8000;
      }
      // Accept both full names ("January") and 3-letter abbreviations ("Jan '25")
    ...
    if (attempt < 2 && e.message.includes('Timeout')) {
      // SPA may not have settled — go back to portal home, wait, then retry once
    ...
    if (!forceRefresh) {
      try {
        ...
        if (pgData) {
          ...
          // Re-warm Redis so next request is even faster
