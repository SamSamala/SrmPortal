...
async function scrapeAll(cookies) {
  ...
  // Known pagesId values from probe — Zoho Creator card view page IDs
  // These are baked into the page HTML as: pagesId="..." displayType="card"
  const KNOWN_PAGES_IDS = {
    ODD:  '2727643000344798001',
    EVEN: '2727643000359102025',
  };

  // Fetch planner data via Zoho Creator's card view section AJAX endpoint.
  // The page shell HTML contains pagesId; the browser JS calls these endpoints to load section data.
  async function fetchPlannerByPagesId(pagesId, pageName) {
    const APP_BASE = 'https://academia.srmist.edu.in/srm_university/academia-academic-services';
    const ajaxHeaders = {
      'Origin': 'https://academia.srmist.edu.in',
      'Referer': `https://academia.srmist.edu.in/#Page:${pageName}`,
      'User-Agent': UA,
      'Cookie': cookieStr,
    ...
    // Try several Zoho Creator internal AJAX endpoint patterns for card view pages
    const attempts = [
      // Pattern 1: zcrf-pages/{pagesId} — common internal page-content endpoint
      { url: `${APP_BASE}/zcrf-pages/${pagesId}`, method: 'GET' },
      // Pattern 2: pages/view with pagesId param
      { url: `${APP_BASE}/pages/view?pagesId=${pagesId}&startIndex=0&pageSize=200`, method: 'GET' },
      // Pattern 3: page-elements fetch
      { url: `${APP_BASE}/page-elements`, method: 'POST', body: `pagesId=${pagesId}&startIndex=0&pageSize=200` },
      // Pattern 4: zcpages with startIndex
      { url: `${APP_BASE}/zcpages?pagesId=${pagesId}&startIndex=0&pageSize=200`, method: 'GET' },
      // Pattern 5: elements/fetch
      { url: `${APP_BASE}/elements/fetch`, method: 'POST', body: `pagesId=${pagesId}&startIndex=0&pageSize=200` },
    ];

    for (const attempt of attempts) {
      try {
        const opts = { method: attempt.method, headers: ajaxHeaders, redirect: 'follow' };
        if (attempt.body) opts.body = attempt.body;
        const r = await fetch(attempt.url, opts);
        const text = await r.text();
        const status = r.status;
        const first200 = text.substring(0, 200).replace(/\s+/g, ' ');
        console.log(`[planner pages/${pagesId}] ${attempt.method} ${attempt.url.replace(APP_BASE, '')} → ${status}: ${first200}`);

        // Check for pageSanitizer HTML (same as normal page but via AJAX)
        const htmlMarker = `pageSanitizer.sanitize('`;
        const si = text.indexOf(htmlMarker);
        if (si !== -1) {
          const cs = si + htmlMarker.length;
          let ce = -1;
          for (let i = cs; i < text.length - 2; i++) {
            if (text[i] === "'" && text[i+1] === ')' && text[i+2] === ';') {
              let bs = 0, j = i-1;
              while (j >= 0 && text[j] === '\\') { bs++; j--; }
              if (bs % 2 === 0) { ce = i; break; }
            }
          }
          if (ce !== -1) {
            console.log('Planner HTML found via pagesId API:', attempt.url.replace(APP_BASE, ''));
            return jsUnescape(text.substring(cs, ce));
          }
        }
        // Check for JSON response
        if (text.trim().startsWith('{') || text.trim().startsWith('[')) {
          console.log(`[planner pagesId JSON] ${attempt.url.replace(APP_BASE, '')}: ${text.substring(0, 500)}`);
        }
      } catch(e) { console.warn('[planner pagesId] attempt failed:', attempt.url, e.message); }
    }
    return null;
  }

  async function fetchPlannerSem(sem) {
    // Try pagesId-based AJAX fetch first (card view pages)
    const pagesId = KNOWN_PAGES_IDS[sem];
    const pageName = _plannerCandidates(sem)[0];
    if (pagesId) {
      const fromApi = await fetchPlannerByPagesId(pagesId, pageName);
      if (fromApi) return fromApi;
    }

    for (const name of _plannerCandidates(sem)) {
      // Try /report/ URL (list-view, parseable via pageSanitizer)
      const fromReport = await fetchReport(cookieStr, name);
      if (fromReport) { console.log('Planner found via /report/:', name); return fromReport; }
    }
