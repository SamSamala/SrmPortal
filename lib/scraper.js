...
async function loginWithBrowser(email, password) {
  ...
  try {
    ...
    await browser.close();
    try { await redis.set(`data:v2:${email}`, JSON.stringify(data), 'EX', 600); } catch(e) {}
...
async function startLogin(email, password, useSession) {
  if (useSession) {
    ...
    try {
      const cachedData = await redis.get(`data:v2:${email}`);
      if (cachedData) {
...
function parsePlanner(text) {
  ...
  // Each group of 5 columns: [Dt, Day, Event/Holiday, DayOrder, Separator]
  for (let i = headerIdx + 1; i < lines.length; i++) {
    ...
    for (const { month, year, groupStart } of months) {
      ...
      if (!isNaN(dayOrder) && dayOrder >= 1 && dayOrder <= 5) {
        result[key] = { order: dayOrder, holiday: null };
      } else if (event) {
        const name = event.replace(/\s*-\s*Holiday\s*$/i, '').trim() || 'Holiday';
