...
async function navigateToPage(page, pageName) {
  ...
  }, pageName, { timeout: (pageName.includes('Academic_Planner') || pageName === 'My_Attendance') ? 35000 : 20000 });
...
function parseMarks(text) {
  ...
  const blocks = text.substring(si).split(/\n(?=\d{2}[A-Z]{2,7}\d{3,4}[A-Z\d]{0,4}\s+(?:Theory|Practical|Lab|Audit))/);

  for (const block of blocks) {
    const cm = block.match(/^(\d{2}[A-Z]{2,7}\d{3,4}[A-Z\d]{0,4})\s+(Theory|Practical|Lab|Audit)/);
    if (!cm) continue;
    const tests = [];
