// Scraper — Playwright-based automation for SRM portal login and data extraction
...
async function loginWithBrowser(email, password) {
  ...
  try {
    ...
    // Use :visible so we skip hidden duplicate password fields (Zoho renders 3, only 1 visible)
    await frame.waitForSelector('input[type="password"]:visible', { timeout: 20000 });
    await frame.fill('input[type="password"]:visible', password);
    await frame.waitForTimeout(500);
    await frame.click('#nextbtn');
...
async function navigateToPage(page, pageName, attempt = 1) {
  console.log(`Navigating to page: ${pageName}${attempt > 1 ? ` (attempt ${attempt})` : ''}`);
  ...
  try {
    await page.waitForFunction((pageName) => {
      ...
      if (pageName === 'My_Attendance') {
        if (!text.includes('Registration Number') || text.length < 800) return false;
        return text.includes('Internal Marks') || text.length > 8000;
      }
      if (pageName.includes('Academic_Planner')) return /(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s*'\d{2}/i.test(text) && !text.includes('My Time Table AY');
      return text.length > 1000;
    }, pageName, { timeout: pageName === 'My_Attendance' ? 30000 : 20000 });
  } catch (e) {
    if (attempt < 2 && e.message.includes('Timeout')) {
      // Full SPA reset — go to portal home, let it settle, then navigate again
      console.warn(`Page "${pageName}" timed out, resetting SPA and retrying…`);
      await page.goto('https://academia.srmist.edu.in/portal/academia-academic-services/', { waitUntil: 'domcontentloaded', timeout: 30000 });
      await page.waitForTimeout(4000);
      return navigateToPage(page, pageName, 2);
    }
    throw e;
  }
...
async function scrapeAll(page) {
  ...
  if (!cur.includes('academia.srmist.edu.in/portal') || cur.includes('redirectFromLogin') || cur.includes('accounts')) {
    ...
    // Always let the SPA settle before navigating to any sub-page,
    // especially after multiple session-termination intermediate pages
    await page.waitForTimeout(3000);
  }
  ...
  // If marks seem incomplete vs attendance, the section may still be loading — retry once
  if (attendance.length > 0 && marks.length < attendance.length) {
    console.log(`Marks possibly incomplete (${marks.length}/${attendance.length} courses). Waiting 4s and retrying…`);
    await page.waitForTimeout(4000);
    const retryText = await page.evaluate(() => document.body.innerText);
    const retryMarks = parseMarks(retryText);
    if (retryMarks.length > marks.length) {
      console.log(`Retry improved marks: ${marks.length} → ${retryMarks.length}`);
      marks = retryMarks;
    }
  }

  // Normalize marks codes to match attendance codes.
  // The portal sometimes omits the trailing letter from a course code in the marks
  // section (e.g. "21DCS201" in marks vs "21DCS201P" in attendance).
  // Build a lookup: attendance code without last char → full attendance code.
  const attCodeMap = {};
  for (const c of attendance) {
    attCodeMap[c.code] = c.code;                  // exact match always wins
    attCodeMap[c.code.replace(/[A-Z]$/, '')] = c.code; // stripped fallback
  }
  marks = marks.map(m => ({
    ...m,
  ...
  // Dynamically compute planner page names for the current academic year.
  // SRM academic year: ODD = Jul-Dec, EVEN = Jan-Jun.
  // Both planners share the same academic year label (e.g. 2025_26).
  const _now = new Date(), _yr = _now.getFullYear(), _mo = _now.getMonth();
  // Academic year start: if Jan-Jun → started _yr-1; if Jul-Dec → started _yr
  const _ayStart = _mo >= 6 ? _yr : _yr - 1;
  const _ayEnd   = String(_ayStart + 1).slice(-2);
  const _planners = [
    `Academic_Planner_${_ayStart}_${_ayEnd}_ODD`,   // Jul-Dec _ayStart
    `Academic_Planner_${_ayStart}_${_ayEnd}_EVEN`,  // Jan-Jun (_ayStart+1)
  ];

  // Load both planners so the full academic year is covered
  let plannerData = {};
  for (let _pi = 0; _pi < _planners.length; _pi++) {
    const plannerPage = _planners[_pi];
    try {
      // Navigate to portal home before each planner to reset SPA state.
      // Without this, SPA hash navigation reuses cached DOM from the previous
      // planner and both fetches return identical content.
      console.log('Resetting SPA state before planner:', plannerPage);
      await page.goto('https://academia.srmist.edu.in/portal/academia-academic-services/', { waitUntil: 'domcontentloaded', timeout: 30000 });
      await page.waitForTimeout(2000);
      ...
      // Sanity check: verify each planner is serving the correct semester's data.
      // SRM sometimes serves the wrong semester's data if the planner isn't published yet.
      // ODD planner should have entries in Jul-Dec (month >= 7).
      // EVEN planner should have entries in Jan-Jun (month <= 6).
  ...
  // Log plannerData date range summary for diagnostics
  if (plannerData) {
...
// Scrapes in background using stored Redis session, updates cache when done.
// Uses a Redis lock to prevent concurrent scrapes for the same user.
async function backgroundRefresh(email) {
  ...
  try {
    ...
    const browser = await chromium.launch({ headless: true, args: ['--no-sandbox','--disable-setuid-sandbox','--disable-dev-shm-usage','--disable-gpu'] });
    const context = await browser.newContext({ userAgent: UA, viewport: { width: 1280, height: 720 }, storageState: JSON.parse(saved) });
    const page = await context.newPage();
    try {
      await page.goto(PORTAL_URL, { waitUntil: 'domcontentloaded', timeout: 30000 });
      await page.waitForTimeout(2000);
      if (page.url().includes('signin') || page.url().includes('accounts')) {
      ...
      await browser.close();
      console.warn('Background refresh scrape failed:', e.message);
    }
  } catch(e) {
    ...
    if (!forceRefresh) {
      try {
        ...
        if (cachedData) {
          ...
          // Kick off background refresh if cache is older than 10 minutes
        ...
        // Postgres fallback (24hr TTL) — serves data when Redis cache has expired
        ...
        if (pgData) {
          ...
          backgroundRefresh(email).catch(() => {}); // always refresh stale postgres data
    ...
    try {
      ...
      if (!url.includes('signin') && !url.includes('accounts')) {
        ...
        try {
          ...
          // Fall back to stale cache rather than logging the user out
