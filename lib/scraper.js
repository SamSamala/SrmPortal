...
async function scrapeAll(page) {
  ...
  for (const plannerPage of _planners) {
    try {
      ...
      // Sanity check: ODD planner should have entries in Jul-Dec (month >= 7).
      // If SRM hasn't published the ODD planner yet, the page serves the EVEN data
      // (same entries, all in Jan-Jun). Detect and discard to avoid overwriting.
      if (plannerPage.includes('_ODD')) {
        const hasOddMonths = Object.keys(entries).some(k => {
          const mo = parseInt(k.split('-')[1]);
          return mo >= 7; // July or later
        });
        if (!hasOddMonths && Object.keys(entries).length > 0) {
          console.warn('ODD planner appears to contain EVEN-semester data — skipping:', plannerPage, '(entries:', Object.keys(entries).length + ', all in Jan-Jun)');
          continue;
        }
      }
