...
// ─── ADD THESE 3 LINES ───────────────────────────────────────────
  console.log('=== MY_TT RAW START ===');
  console.log(myTTText.substring(0, 3000));
  console.log('=== MY_TT RAW END ===');
  // ────────────────────────────────────────────────────────────────
...
function parseSlotMap(text) {
  ...
  console.log('parseSlotMap: total lines:', lines.length);
  console.log('parseSlotMap: first 10 lines:', lines.slice(0, 10));

  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];

    // Must start with a row number followed by a course code
    if (!/^\d+\s+\d{2}[A-Z]{2,5}\d{3}/.test(line)) continue;

    // Extract course code and name
    const cm = line.match(/^\d+\s+(\d{2}[A-Z]{2,5}\d{3}[A-Z]{0,3})\s+(.+?)\s+\d+\s+(?:Regular|Elective|Mandatory)/);
    if (!cm) {
      console.log('parseSlotMap: no course match on line:', line.substring(0, 120));
      continue;
    }

    const type = line.includes('Practical') ? 'Practical' : 'Theory';

    // Extract slot string — look for pattern like (N) SLOTNAME or slot after credits
    const slotMatch = line.match(/\(\d+\)\s*([\w-]+)\s+/);
    if (!slotMatch) {
      console.log('parseSlotMap: no slot match on line:', line.substring(0, 120));
      continue;
    }

    // Extract room — text between slot and AY
    const roomMatch = line.match(/\(\d+\)\s*[\w-]+\s+(.+?)\s+AY/);
    ...
    console.log('parseSlotMap: code:', cm[1], '| slots:', slots);
  ...
  console.log('parseSlotMap: final map keys:', Object.keys(slotMap));
  return slotMap;
}
