...
async function trackUser(email) {
  try {
    ...
    // Record last-active timestamp for 24h activity tracking
    await redis.zadd('srm:users:active', Date.now(), email);
  } catch(e) {
...
function htmlToText(html) {
  ...
  // Expand colspan cells so that a lab cell spanning N periods
  // appears N times in the extracted text (fixing 3-period labs showing as 2).
  $('td[colspan], th[colspan]').each(function () {
    const colspan = parseInt($(this).attr('colspan')) || 1;
    if (colspan > 1) {
      const inner = $(this).html();
      const extras = [];
      for (let i = 1; i < colspan; i++) extras.push(`<td>${inner}</td>`);
      $(this).after(extras.join(''));
  ...
  for (const block of blocks) {
    ...
    for (let hi = 0; hi < allHeaders.length; hi++) {
      ...
      const scored = scoreMatch
        ? (scoreMatch[1] === 'Abs' ? null : Math.round(parseFloat(scoreMatch[1]) * 100) / 100)
        : null;
    ...
    if (missingScores > 0 && missingScores >= allHeaders.length / 2 && allHeaders.length > 0) {
      ...
      if (scoreTokens.length > 0) {
        ...
        for (let hi = 0; hi < allHeaders.length; hi++) {
          ...
          const scored = raw === 'Abs' ? null : (raw.match(/^[\d.]+$/) ? Math.round(parseFloat(raw) * 100) / 100 : null);
          tests.push({ name: h.name, maxMarks: h.maxMarks, scored });
        }
      }
    }
