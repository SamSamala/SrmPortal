...
async function loginWithBrowser(email, password) {
  ...
  try {
    ...
    await browser.close();
    try { await redis.set(`data:v4:${email}`, JSON.stringify(data), 'EX', 3600); } catch(e) {}
...
async function scrapeAll(page) {
  ...
  // Load both EVEN and ODD planners so the full academic year is covered
  let plannerData = {};
  for (const plannerPage of ['Academic_Planner_2025_26_EVEN', 'Academic_Planner_2025_26_ODD']) {
    try {
...
async function startLogin(email, password, useSession, forceRefresh = false) {
  if (useSession) {
    ...
    if (!forceRefresh) {
      try {
        const cachedData = await redis.get(`data:v4:${email}`);
        if (cachedData) {
